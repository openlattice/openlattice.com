/*
 * @flow
 */

/*
 * neutrals
 */

export const N0 :string = '#ffffff';
export const N1 :string = '#f9f9fd';
export const N2 :string = '#6f788a';
export const N3 :string = '#4d4d4d';
export const N4 :string = '#000000';
export const N5 :string = '#b9b7bf';
export const N6 :string = '#F9F9FC';
export const N7 :string = '#3B3B49';
export const N8 :string = '#505060';
export const N9 :string = '#9999AF';

/*
* pinks
*/

export const PK0 :string = '#ff5e95';

/*
 * purples
 */

export const PP0 :string = '#f8f8fc';
export const PP1 :string = '#eeeef9';
export const PP2 :string = '#c881ff';
export const PP3 :string = '#9359ff';
export const PP4 :string = '#6124e2';
export const PP5 :string = '#31275c';
export const PP6 :string = '#7c7791';
export const PP7 :string = '#7860FF';
