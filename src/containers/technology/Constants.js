/* eslint-disable max-len */

import IntegrationModelIcon from '../../assets/images/standard-data-icon.svg';
import LinkIcon from '../../assets/images/link-across-icon.svg';
import ShareIcon from '../../assets/images/share-manage-icon.svg';
import DiscoverIcon from '../../assets/images/discover-icon.svg';
import ComplianceIcon from '../../assets/images/compliance-icon.svg';
import DeidentificationIcon from '../../assets/images/deidentification-icon.svg';
import EncryptionIcon from '../../assets/images/encryption-icon.svg';
import OpenSourceIcon from '../../assets/images/open-source-icon.svg';


export const title1 = 'Standardized Data Integrations';
export const body1 = 'Standardized models streamline sharing of data and analyses across organizations.';
export const icon1 = IntegrationModelIcon;

export const title2 = 'Record Linkage';
export const body2 = 'Machine Learning-backed linking mechanism for matching individuals across datasets.';
export const icon2 = LinkIcon;

export const title3 = 'Share & Manage Access';
export const body3 = 'Share datasets with researchers and other jurisdictions. Grant and revoke access securely and efficiently.';
export const icon3 = ShareIcon;

export const title4 = 'Discover';
export const body4 = 'Find relevant datasets by searching the data model and its documentation. Filter by property and entity types.';
export const icon4 = DiscoverIcon;

export const title5 = 'Compliance';
export const body5 = 'Data is stored in HIPAA, 42 CFR Part 2, and CJIS compliant infrastructure.';
export const icon5 = ComplianceIcon;

export const title6 = 'De-identification';
export const body6 = 'Manage sharing of de-identified datasets with researchers or the public.';
export const icon6 = DeidentificationIcon;

export const icon7 = OpenSourceIcon;

export const icon8 = EncryptionIcon;
